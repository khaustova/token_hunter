...
from ..models import Status, Transaction, Type
...
class CoinChecker:
    ...
    def check_price_change(self):
        if self.coin_data["priceChange"]["m5"] < -5 or self.coin_data["priceChange"]["m5"] > 100:
            logger.debug(f"⨉ Стоимость монеты {self.coin_name} падала больше, чем на 10 % или росла больше, чем на 100 %")
            return False
    ...
    def check_socio(self):
        ...
        socio_info = self.coin_data.get("info", None)
        if socio_info:
    ...
    def check_fdv(self):
        if self.coin_data["fdv"] >= 100000000:
            return True
        
        return False
    
    
    def prebuy_check(self, total_transfers):
        total_transactions = self.coin_data["txns"]["h1"]["buys"] + self.coin_data["txns"]["h1"]["buys"]
        if ((total_transfers >= 10 and total_transfers <= 5000) and 
            (total_transactions >= 10 and total_transactions <= 200) and
            self.check_socio() and
            self.check_price_change() and
            self.coin_data["volume"]["m5"] >= 100
        ):
                return True
...
def buy_coin(pair, total_transfers, coin_name, coin_address):
    ...
    prebuy_coin_checker = CoinChecker(pair)
    ...
    transaction = Transaction.objects.get_or_create(
        ...
        buying_transactions_sells_h1=coin_data["txns"]["h1"]["sells"],
